...
myMAplot_limD <- function(deseq_results, fdr = 0.1, fc = 1.5, ylim, t) {
  ...
  ggmaplot(deseq_results,
    ...
    theme(axis.title = element_text(size = 13), legend.text = element_text(size = 11)) +
    geom_hline(yintercept = c(-fdr, fdr) , linetype = "dashed") + 
    scale_color_manual(labels = c(paste0("upregulated Genes: ", sum(sig == 1)), 
                                  paste0("downregulated Genes: ", sum(sig == 2)),
                                  ""),
                       values = c(cols$col_upregulated, cols$col_downregulated, "darkgrey"),
                       guide = guide_legend(override.aes = list(size = 4,
                                                                color = c(cols$col_upregulated, cols$col_downregulated, NA))))
...
#angepasstes VolcanoPlot auf DAS

myDASVolcanoPlot <- function(deseq_results, data_type, pCutoff = 0.01, FCcutoff = 0.5,
                          xlim = c(min(deseq_results[['V2']], na.rm = TRUE) - 1.5, max(deseq_results[['V2']], na.rm = TRUE) + 1.5), 
                          ylim = c(0, max(-log10(deseq_results[['V3']]), na.rm = TRUE) + 0.5),
                          lab_selection = NULL, symbol_type , titel, subtitel) {
...
myGObarplot_new <- function(panther_result_up, panther_result_down, count_threshold, fdr_threshold, num_tops = 10, 
  ...
  #View(panther_result_up)
  #View(panther_result_down)
  #View(background)
