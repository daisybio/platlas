      ...
      fluidRow(
       ...
       # height = "00px",
        ...
        box(
          ...
          "The shown results of the functional enrichment analysis were retrieved from the use of GOSeq in R (Young MD et al. (2010)) and are browsable by the TPM (transcript per million) read count normalization threshold of the original data, the patient datasets, the desired ontology (Biological Process, Cellular Component and Molecular function) and the GO-Entries discovered on upregulated genes or downregulated genes in both filters.",br(),br(),
          "The representation of these results was done with a downloadable data table of the discovered GO- Entries, which when clicked lead to the Amigo Gene Ontology database and a barplot showing the top 10 Entries classified by their higher fold change."
        )
...
server <- function(input,output){
  ...
  FER <- eventReactive(c(input$clickGO, input$clickGO2),ignoreInit = T,{
    tabBox(
      ...
      title = "Get the results",
      tabPanel(
        id="GOtab",title = "Data Table", solidHeader = TRUE,
      ...
      tabPanel(
        ...
        "increased/decreased categories and their terms in relation to upregulated and downregulated genes in the corresponding category",
        br(),
      ...
      tabPanel(
        ...
        "The z-score, computed by the circ_dat() Method from the GOplot package, is a score meant to give an orientation to understand if a category is more likely to be increased (z-score = positive) or decreased (z-score = negative) ",
        br(),
        withMathJax(),
  ...
  Das <- eventReactive(input$clickDAS1,{
    tabBox(
      ...
      tabPanel(
        ...
        selectInput(inputId = "DASUD", label = "choose significantly alternatively spliced LSV list in Condition 1 (left dataset) or Condition 2 (right dataset)", choices = c("Condition 1", "Condition 2"), multiple = FALSE),
...
#pGO2 <- "./www/"
  ...
  # UDA <-  eventReactive(c(input$clickGO,input$clickGO2),{
  #   #print("in UDA")
  #   fin <-""
  #   circ <-""
  #   im <- 
  #   if(clickedUDA1() < input$clickGO){
  #     if(as.character(input$UDA) == "upregulated"){
  #     fin<-path3U
  #     circ <- pathCU
  #   }else if(as.character(input$UDA) == "downregulated"){
  #     fin<-path3D
  #     circ <- pathCD
  #   }else if(as.character(input$UDA) == "all"){
  #     fin<-path3A
  #     circ <- pathCA
  #   }
  #   }else if(clickedUDA2() < input$clickGO2){
  #     if(as.character(input$UDA2) == "upregulated"){
  #     fin<-path3U
  #     circ <- pathCU
  #   }else if(as.character(input$UDA2) == "downregulated"){
  #     fin<-path3D
  #     circ <- pathCD
  #   }else if(as.character(input$UDA2) == "all"){
  #     fin<-path3A
  #     circ <- pathCA
  #   }
  #   }
  #   getfin<- fin
  #   endlist <- list(getfin,circ)
  # }
  # )
...
output$VPMI <- renderPlot({
 ...
 output$KPMnet <- renderForceNetwork({
  ...
  output$KPMGOnet <- renderForceNetwork({
    ...
    forceNetwork(Links = edges, Nodes = nodestab[, c(1,2,5,7)],
                 Source = "from", Target = "to",
                 Value = "width", NodeID = "id", Group = "type",opacity = 0.9,fontSize = 10 ,zoom = TRUE)
...
#  kp_p = reactiveVal("")
  ...
  DASDataTable <- eventReactive(input$clickDASDT,{
    ...
    l <- NULL
    e <- NULL
    if(as.character(input$DASUD) == "Condition 1"){
      l <-  DAS_sigtab()[DAS_sigtab()$V2 < 0, ]
      l <-  l[order(l$V2, decreasing = FALSE),] 
      DAS_end_sig_tab(l)
      e <- l
    }else if(as.character(input$DASUD) == "Condition 2"){
